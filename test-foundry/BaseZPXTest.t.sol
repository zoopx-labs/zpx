...
abstract contract BaseZPXTest is Test {
    ...
    address internal rescuer = address(0xD00D); // unused with current initializer, retained for future extensions
    ...
    function deployV1(uint256 /*cap*/ ) internal {
        ...
        minters[0] = minter;
        bytes memory data = abi.encodeCall(ZPXV1.initialize, (admin, minters, pauser));
        LocalERC1967Proxy proxy = new LocalERC1967Proxy(address(implV1), data);
        token = ZPXV1(address(proxy));
    }

    function upgradeToV2(address bridge) internal returns (ZPXV2 proxyAsV2) {
