...
async function fixtureV2() {
  ...
  const initCalldata = encodeFunctionData({ abi: ZPXV1.abi as any, functionName: 'initialize', args: [admin.account.address, [minter.account.address], pauser.account.address] });
  ...
  const contract = { address: proxyAddress, abi: ZPXV1.abi };
  return { publicClient, proxyAddress, admin, minter, pauser, user1, user2, mockBridge, contract };
}
